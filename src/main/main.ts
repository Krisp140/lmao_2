...
import type { Libp2pType } from '../renderer/context/ctx'
...
let libp2p: Libp2pType | null = null
...
async function initLibp2p() {
  try {
    ...
    libp2p.services.directMessage.addEventListener('message', (evt: any) => {
...
function setupIPC() {
  // Connect to peer
  ipcMain.handle('connect-to-peer', async (_, multiaddrStr: string) => {
    try {
